...
\item \tsurv{a}{i,j} -- for article $a \in \articles$, measures the
    amount of text introduced in revision \version{i}
    that survives to \version{j}.

\item \judges{a}{n}{i} -- a function that returns the set of
    ...
    to \version{i} in time.  See Definition~\ref{def:judges}.

\end{itemize}

\subsection{Chapter~\ref{ch:editquality} -- Contribution Quality}

\begin{itemize}

\item \quality{tdecay}{n}{k} -- the text decay quality of revision
    \version{k} (we leave the article to which \version{k} belongs
    ...
    \mynote{Make article explicit, as with elong!}

\item \quality{elong}{a,n}{k} -- the edit longevity quality of
    revision $\version{k} \in \article{a}$, as judged by up to
    $n$ judging revisions after \version{k}.
