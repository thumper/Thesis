...
\item[\diff1]
    ...
    \begin{description}
    \item[Includes:]
        \opt{min wordlen}, \opt{max matches},
    \item[Excludes:]
        \opt{header/trailer}.
        \opt{longest match}, \opt{prev matches}.
    \end{description}
...
\item[\diff2]
    ...
    suggested by Obst~\cite{Obst1987} and
    Reichenberger~\cite{Reichenberger1991}.
    Also includes the \opt{header/trailer} optimization.
    \begin{description}
    \item[Includes:]
        \opt{min wordlen}, \opt{max matches}, \opt{header/trailer}
    \item[Excludes:]
        \opt{longest match}, \opt{prev matches}.
    \end{description}
...
\item[\diff3]
    ...
    the globally \opt{longest match} between the source and target
    revisions.
    Without the \opt{prev matches} optimization, this version of the algorithm
    uses an associative array to track the matches made so far.
    %
    \begin{description}
    \item[Includes:]
	\opt{min wordlen}, \opt{max matches},
	\opt{header/trailer}, \opt{longest match}.
    \item[Excludes:]
	\opt{prev matches}.
    \end{description}
...
\item[\diff4]
    ...
    We believe that the \opt{header/trailer} optimization generates
    ...
    than \diff5, which includes that optimization.
    %
    \begin{description}
    \item[Includes:]
	\opt{min wordlen}, \opt{max matches}, \opt{longest match},
	\opt{prev matches}.
    \item[Excludes:]
	\opt{header/trailer}.
    \end{description}
...
\item[\diff5]
    This is an extension of \diff4 that includes the
    \opt{header/trailer} optimization.
    %
    \begin{description}
    \item[Includes:]
	\opt{min wordlen}, \opt{max matches},
...
\item[\diff6] This is the original difference algorithm
...
\item[\diff7]
    ...
    production
    WikiTrust system.
    It is the same as \diff6, with the addition of the
    \opt{header/trailer} optimization.
    %
    \begin{description}
    \item[Includes:]
	\opt{min wordlen}, \opt{max matches}, \opt{longest match},
	\opt{prev matches}, \opt{header trailer}.
    \item[Excludes:] \textit{none}.
    \end{description}
    %
...
\item[\diff8]
    ...
    for the \opt{header/trailer} optimization.
