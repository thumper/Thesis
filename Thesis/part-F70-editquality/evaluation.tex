...
\mynote{NumRevs is how many PAN2010 revisions we made predictions for.
...
includes a division.
It would be better, probably, if it skipped the calculation if the
size of the \textit{edit script} is one (meaning that it must be
a single block copy).
If the edit distance function returns zero, I believe that it
would be better for the quality function to also return zero.}

The typical measure used to compare difference algorithm is the
running time, and so we include this value.
The running time is not really affected by either the
match quality or the edit distance calculations,
except for algorithm \textbf{diff6}
(see Table~\ref{tab:difftiming}), which has a running
time which climbs up to approximately 418 minutes
when used with match qualities \textbf{mq5}, \textbf{mq6},
or \textbf{mq7}.
Table~\ref{tab:difftiming} reveals that Reichenberger's
difference algorithm runs in half the time (or more) that the
WikiTrust algorithms complete in,
and can also predict vandalism very competitively
(Table~\ref{tab:editshoutA}).
\mynote{Diff7 and diff8 not calculated yet.}

\input{part-F70-editquality/quality-table-timing}

\mynote{I included the total number of triangles calculated.
Like NumRevs, this number is dependent on the edit distance
from the previous rev being non-zero.
More interesting is the number of bad triangles; a bad triangle
is one that doesn't satisfy the triangle inequality.
What is interesting to note about this column is that
it's generally correlated with the PR-AUC.
Is it necessary for the edit distance to not respect the
triangle inequality in order for the edit longevity to make
good predictions?
