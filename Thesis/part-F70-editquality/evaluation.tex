...
Each formula for edit distance we have defined so far
\mynote{where do we define any?  give references}
 is calculated
...
\item[\textbf{diff3}]
    This is a WikiTrust variation of difference algorithm, which uses prefers
    the globally \textbf{longest match} for a given starting position in the target revision.
...
\mynote{which table?} do not agree with those expectations.
\begin{itemize}
...
\mynote{Does longest match actually save CPU as we say when we define
it?}

\mynote{Does text longevity actually change when +c or -c?}
