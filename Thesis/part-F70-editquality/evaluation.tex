...
The reason for the discrepancy is an interaction between long matches
and our rule to not look at over-frequent matches (greater than 50).
The case I found, there was a 1400 word match at the beginning of the
article.  For hashing, that entire segment was marked as matched.
In prev-matches, an overly common word pair results in no matches,
so that the skipmatch test doesn't correctly determine that we've
already investigated a longer match.  This results in many more
items in the priority queue than are necessary.}
\mynote{ But they should be items which will only be rejected later.
Why does the diff come out differently?  I can't remember now...
}
...
idea, for even better speedup.  Hmm, matchskip didn't speed
up; it seems to have slowed things down.  Rollback?}
