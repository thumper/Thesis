...
Our difference algorithm makes use of
...
computing edit distance; the goal of our work differs from this
...
(see Figure~\ref{fig:match-comparison}), making it unlikely that our
...
\caption[An example of when WikiTrust and Tichy differ in matching.]{
  ...
  The WikiTrust algorithm describes the match as three operations, while
  the Tichy algorithm is able to match with two operations.
...
alternative analysis when a word match is lost because of the minimum
words requirement) so that the full triangle inequality also holds true.
...
inequality is desirable, it is neither necessary nor sufficient to
