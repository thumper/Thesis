...
retain, or remove, the changes performed by $A_i$ in bringing
...
We therefore define \judges{a}{n}{i} to return the set of up to $n$
\textbf{next} revisions after \version{i},
such that the author of each $\version{} \in \judges{a}{n}{i}$
...
where only high-reputation authors are selected as judges.
