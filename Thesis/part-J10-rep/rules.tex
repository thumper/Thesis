...
There are two behaviors that we chose to promote as desirable:
...
We note that the scope of potential future work around possible
signals is quite large, as there are many other behaviors that
might be desirable in promoting the general Wikipedia community,
and in fact many of those behaviors have been enumerated in the form
of \intro{Barnstars}.\footnote{\url{http://en.wikipedia.org/wiki/Wikipedia:Barnstars}}
...
and use a text differencing algorithm to compute these values.
...
\label{rule-text}
  ...
  \]
  where $j = \revpos{v}$ is the version position of $v$ in \versions{a}
...
is undone (value $-1$) or preserved (value $+1$) through version \version{j}. 

Note that $\esurv{a}{i,i-1,j} < 0$ (i.e., \revauthor{\version{j}}
votes to lower the reputation of \revauthor{\version{i}}), only when
...
\label{eq-edit-long}
\end{equation*}
Since this value is undefined when \dist{a}{i-1,i} = 0,
