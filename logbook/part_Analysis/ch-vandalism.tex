...
\program{run-expt.sh} program for \file{project/reputation-quality}.
This takes a long time, and the output is buffered by
\file{batch\_process.py}, but it eventually completes.
...
The \program{splitwiki} portion finished early this week, and then the
\program{combinestats} part began.
...
I spent some time looking at the \program{combinestats} program,
...
I created five volumes of size 250GB, and created the software raid
array like so:
...
Also, while thinking about my current results, I realized that we only
