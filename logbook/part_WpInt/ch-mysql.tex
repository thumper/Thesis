...
Something that I have not mentioned in previous entries is that
...
To fix this, edit the file in \file{/etc/apparmor.d} for \program{mysqld}.
Then be sure to restart \package{apparmor} via its init script.
...
Looking into \file{daemon.log}, the messages indicated that some
...
I then erased all the \mysql files in \file{/store/lib/mysql} and
\file{/raid/mysql/logdir}; these directories were mentioned in
the config files.
...
\end{verbatim}
Well, actually that does seem right.
For some reason, I thought that the whole database was on a
single drive.
