...
foreach my $page (@{ $ref->{page} }) {
  for (my $i = 0; $i < @{ $page->{revision} }; $i++) {
    ...
    my $d1 = WikiTrust::BasicTextTracking->new();
    my $d2 = WikiTrust::FasterTextTracking->new();
    my $words = $d1->target($text, $revid);
    $d2->target($words, $revid);

    my ($w1, $w2);
    timethis(1, sub { $w2 = $d2->track_text($prevrevs); }, "FasterTextTracking");
    timethis(1, sub { $w1 = $d1->track_text($prevrevs); }, "BasicTextTracking");
    if (compareResults($w1, $w2)) {
	my $size = scalar(@$w1);
	my $back = scalar(@$prevrevs);
	print "revid $revid  :: SAME on $size words with $back prev revs\n";
    } else {
	print "revid $revid  :: DIFFERENT\n";
	print "BasicTracking: ", Dumper($w1), "\n";
	print "FasterTracking: ", Dumper($w2), "\n";
    }
