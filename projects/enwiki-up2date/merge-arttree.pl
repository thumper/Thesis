...
use constant OUTDIR => "/big/thumper/ensplit5-20080103/";
...
sub mergeFDownload {
    ...
    return if ($title =~ m/(?:Media|Special|Talk|User|User_talk|Wikipedia|Wikipedia_talk|Image|Image_talk|MediaWiki|MediaWiki_talk|Template|Template_talk|Help|Help_talk|Category|Category_talk|Portal|Portal_talk):/);
    return if $title eq '';
    ...
    exec("rsync -avq $file $outdir/$pageid.gz") || die "exec: $!";
    exit(1);
